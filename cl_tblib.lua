...
//Excerpted from https://github.com/SneakySqwuid/Circles/blob/circles!-2/circles.lua
...
TBLibUtil.DrawCircle = function(x, y, r, col, rotation, start_angle, end_angle, distance)
    x = tonumber(x) or 0
	y = tonumber(y) or 0
	r = tonumber(r) or 16
...
TBLibUtil.DrawArc = function(x, y, ang, p, rad, color, seg)
	seg = seg || 80
    ang = (-ang) + 180
    local circle = {}
...
TBLibUtil.LerpColor = function(frac, from, to)
	return Color(
...
TBLibUtil.HoverFunc = function(s) return s:IsHovered() end
TBLibUtil.HoverFuncChild = function(s) return s:IsHovered() || s:IsChildHovered() end
...
classes.SetupTransition = function(pnl, name, speed, fn)
	...
	pnl:On("Think", function(s)
		s[name] = Lerp(RealFrameTime()*speed, s[name], fn(s) && 1 || 0)
	end)
end
...
classes.FadeHover = function(pnl, col, speed, rad)
    ...
    pnl._FadeHover = 0
	pnl:SetupTransition("_FadeHover", speed, TBLibUtil.HoverFunc)
	pnl:On("Paint", function(s, w, h)
		local col = ColorAlpha(col, col.a*s._FadeHover)
...
classes.BarHover = function(pnl, col, height, speed)
	...
	pnl:SetupTransition("BarHover", speed, TBLibUtil.HoverFunc)
...
classes.FillHover = function(pnl, col, dir, speed, mat)
	...
	pnl:SetupTransition("FillHover", speed, TBLibUtil.HoverFunc)
	pnl:On("PaintOver", function(s, w, h)
		surface.SetDrawColor(col)
...
classes.BShadow = function(pnl, intensity, spread, blur)
	pnl:On("Paint", function(s, w, h)
		local x, y = s:LocalToScreen(0,0)
		BSHADOWS.BeginShadow()
			draw.RoundedBox(pnl.Radius, x, y, s:GetWide(), s:GetTall(), color_black)
		BSHADOWS.EndShadow(intensity, spread, blur)
	end)
end
...
classes.CircleHover = function(pnl, col, speed, trad)
	...
	pnl:SetupTransition("CircleHover", speed, TBLibUtil.HoverFunc)
	pnl:On("Think", function(s)
		if(s:IsHovered()) then
...
classes.CircleFadeHover = function(pnl, col, speed)
	...
	pnl:SetupTransition("CircleFadeHover", speed, TBLibUtil.HoverFunc)
	pnl:On("Paint", function(s, w, h)
		draw.NoTexture()
...
classes.CircleExpandHover = function(pnl, col, speed)
	...
	pnl:SetupTransition("CircleExpandHover", speed, TBLibUtil.HoverFunc)
	pnl:On("Paint", function(s, w, h)
...
/*---------------------------------------------------------------------------
	TBLib function which adds all the classes to your panel
---------------------------------------------------------------------------*/
...
function meta:TBLib()
	self.Class = function(pnl, name, ...)
		...
		assert(class, "[TBLib]: Class "..name.." does not exist.")
